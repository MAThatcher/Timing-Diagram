...
public class Priority{
  ...
  //our constructor
  public Priority (Task[] ourProcesses){
    tasks = ourProcesses;
    ...
    for (int i = 1 ; i < output[0].length ; i++){
      ...
      for (int indexK = 0 ; indexK < tasks.length ; indexK++){          
        if (tasks[indexK].getArrival() <= i-1 && tasks[indexK].getExternalPriority() < tasks[k].getExternalPriority() && tasks[indexK].getServiceTimeRemaning() != 0){
          k = indexK;
        }
      }
